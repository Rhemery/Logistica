...
export function loadItems() {
  ...
  const itemsList = Item.getList();
  const itemsListSize = itemsList.size();

  let index = 0;
  itemsList.forEach((itemStack) => {
    logProgress("Items", index, itemsListSize);
    index++;
...
export function getItem(id: ItemId): Item | undefined {
  if (!isItemId(id)) {
    /*console.warnf(
      `Expected item ID ('namespace:item'), got ${typeof id === "object" ? JSON.stringify(id) : id}`,
    );*/
    return undefined;
  }
