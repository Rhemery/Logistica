...
export const VILLAGE_REGISTER_CHUNK_RADIUS = 3;
...
export function getVillageDemandPool(): ItemId[] {
  ...
  if (size === global.villagePoolCache.size) {
    return global.villagePoolCache.entries;
  }

  global.villagePoolCache.entries = Object.entries(entries)
    .filter(([itemId, entry]) => {
  ...
  global.villagePoolCache.size = size;
  return global.villagePoolCache.entries;
}
...
BlockEvents.placed(VILLAGE_MARKET_BLOCK_ID, (event) => {
  ...
  if (!owner) {
    event.cancel();
    event.player?.tell({
      text: "Village Market Controller placement requires a player owner.",
      color: "red",
    });
    return;
  }

  console.infof(`[Village Market] Placed by ${owner}`);
  const result = isOutpostPlaceable(
    {
      event,
      state: state,
      block: event.block,
      blockId: VILLAGE_MARKET_BLOCK_ID,
      owner,
    },
    OUTPOST_PLACE_RULES[VILLAGE_MARKET_BLOCK_ID],
  );
  ...
  if (!result.placeable) {
    ...
    if (result.failedRule.id === "village_chunk_detected") {
      event.player.tell({
